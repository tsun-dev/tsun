        ...
        html lang="en" {
            ...
            body {
                div class="container" {
                    ...
                    @if report.is_mock() {
                        section class="mock-banner" {
                            strong { "⚠ Mock engine — these findings are fabricated." }
                            p { "No scan was performed against the target. Run with --engine zap for a real scan." }
                        }
                    }
                    ...
                    section class="summary" {
                        ...
                        div class="summary-grid" {
                            ...
                            div class="summary-card info" {
                                div class="summary-number" { (report.info_count()) }
                                div class="summary-label" { "Info" }
                            }
                    ...
                    section class="vulnerabilities" {
                        ...
                        @if report.alerts.is_empty() {
                            p class="no-issues" { "✓ No vulnerabilities found!" }
                        } @else {
                            div class="alerts" {
                    ...
                    @if !report.suppressed.is_empty() {
                        section class="suppressed" {
                            h2 { "Suppressed Findings (" (report.suppressed.len()) ")" }
                            p class="suppressed-note" {
                                "Matched an ignore rule. Excluded from counts and exit-code gating."
                            }
                            table {
                                thead {
                                    tr {
                                ...
                                tbody {
                                    @for alert in &report.suppressed {
                                        tr {
                                            td { (get_risk_label(alert.severity())) }
                                            td { (alert.alert) }
                                            td class="suppressed-url" { (alert.url) }
                                            td {
                                                (alert
                                                    .suppressed_by
                                                    .as_ref()
                    ...
                    footer class="footer" {
                        p { "Generated by Tsun Security Scanner" }
                        @if report.alerts.iter().any(|a| a.cvss_estimated) {
                            p class="cvss-note" {
                                "CVSS scores are Tsun estimates derived from ZAP risk and confidence, not vendor-assigned scores."
                            }
                        }
                    }
                }
...
fn render_alert(alert: &Alert) -> Markup {
    let risk_class = get_risk_class(alert.severity());
    let confidence_class = get_confidence_class(&alert.confidence);
    let risk_label = get_risk_label(alert.severity());
...
fn get_risk_class(severity: Severity) -> &'static str {
    match severity {
...
fn get_risk_label(severity: Severity) -> &'static str {
    match severity {
        Severity::Critical => "CRITICAL",
        Severity::High => "HIGH",
        Severity::Medium => "MEDIUM",
